...
\begin{compactdesc}
	\item[Orthogonal:] (i.e. cols are orthonormal!) $\mathbf{A}^{-1} = \mathbf{A}^\top$, $\mathbf{A} \mathbf{A}^\top = \mathbf{A}^\top \mathbf{A} = \mathbf{I}$, $\operatorname{det}(\mathbf{A}) \in \{+1, -1\}$, $\operatorname{det}(\mathbf{A}^\top \mathbf{A}) = 1$, $\|\mathbf{Ax}\|^2 = \|\mathbf{x}\|^2$
	...
	\begin{inparaitem}[\color{red}\textbullet]
		...
		\item $\langle \mathbf{x}, \mathbf{y} \rangle \mathbf{y} = (\mathbf{y} \mathbf{y}^\top) \mathbf{x}$  
		\item $\langle \mathbf{x}, \mathbf{y} \rangle = \|\mathbf{x}\|_2 \cdot \|\mathbf{y}\|_2 \cdot \cos(\theta)$
		\item If $\mathbf{y}$ is a unit vec then $\langle \mathbf{x}, \mathbf{y} \rangle$ projects $\mathbf{x}$ onto $\mathbf{y}$
	\end{inparaitem}
...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item $\|\mathbf{x} - \mathbf{y}\|^2 = \langle \mathbf{x} - \mathbf{y}, \mathbf{x} - \mathbf{y} \rangle$
	\item $\mathbf{M} \in \mathbb{R}^{m \times n}$,\ $\|\mathbf{M}\|_F =\allowbreak \sqrt{\sum_{i=1}^{m} \sum_{j=1}^{n} m_{i,j}^2} =\allowbreak \sqrt{trace(\mathbf{M}^\top\mathbf{M})} =\allowbreak \sqrt{\sum_{i=1}^{\min\{m, n\}} \sigma_i^2}$
...
\begin{inparaitem}[\color{red}\textbullet]
  ...
  \item $\frac{\partial}{\partial \mathbf{x}}(\mathbf{x}^\top \mathbf{x}) = 2\mathbf{x} = \frac{\partial}{\partial \mathbf{x}}(\|\mathbf{x}\|^2_2)$
...
Eigenval Problem: $\mathbf{Ax} = \lambda \mathbf{x}$.
\begin{inparaenum}[\color{red} 1.]
	...
	\item $\forall \lambda_i$:
		solve $(\mathbf{A} - \lambda_i \mathbf{I}) \mathbf{x}_i = \mathbf{0}$, $\mathbf{x}_i$ is the $i$-th eigenvec.
	\item (opt.) normalize eigenvec $q_i$: $q_i^{\text{norm}} = \frac{1}{\|q_i\|_2} q_i$.
\end{inparaenum}
...
\begin{inparaenum}[\color{red} 1.]
	...
	\item Eigenval Decomp.: $\boldsymbol{\Sigma} = \mathbf{U} \boldsymbol{\Lambda} \mathbf{U}^\top$, sort eigenvals (and eigenvecs) in descending order
	\item Select $K < D$, keep only the first $K$ eigenvals and corresponding eigenvecs $\Rightarrow \mathbf{U}_K, \boldsymbol{\lambda}_K$
...
\section{SVD}
\begin{inparaitem}[\color{red}\textbullet]
	\item $\mathbf{A} = \mathbf{U} \mathbf{D} \mathbf{V}^\top = \sum_{k=1}^{\operatorname{rank}(\mathbf{A})} d_{k,k} \mathbf{u}_k (\mathbf{v}_k)^\top$
	...
	\item $\mathbf{U}^\top \mathbf{U} = I = \mathbf{V}^\top \mathbf{V}$ ($\mathbf{U}, \mathbf{V}$ cols are orthonormal)
	\item $\mathbf{U}$ cols are eigenvecs of $\mathbf{A} \mathbf{A}^\top$, $\mathbf{V}$ cols are eigenvecs of $\mathbf{A}^\top \mathbf{A}$, $\mathbf{D}$ diagonal elems are singular values, i.e. the square roots of the eigenvals ($\mathbf{A}^\top \mathbf{A}$ and $\mathbf{A} \mathbf{A}^\top$ have the same eigenvals)
	...
	\item Missing cols in $\mathbf{U}$ are basis of $\operatorname{null}(A^\top)$ and in $\mathbf{V}$ are basis of $\operatorname{null}(A)$. Calculate: $\mathbf{A}^\top \mathbf{u} = \mathbf{0}$ or $\mathbf{A} \mathbf{v} = \mathbf{0}$ for $\mathbf{u}$ or $\mathbf{v}$.
\end{inparaitem}

\begin{inparaenum}[\color{red} 1.]
	\item calculate $\mathbf{A}^\top \mathbf{A}$.
	\item calculate eigenvals of $\mathbf{A}^\top \mathbf{A}$, the square root of them, in descending order, are the diag elems of $\mathbf{D}$.
	\item calculate eigenvecs of $\mathbf{A}^\top \mathbf{A}$ using the eigenvals resulting in the cols of $\mathbf{V}$.
	\item calculate the missing matrix $\mathbf{U}$. ($\mathbf{u}_i = \frac{1}{\sqrt{\lambda_i}} \tilde{\mathbf{u}}_i$, $\mathbf{AV} = (\tilde{\mathbf{u}}_1| \dots)$) (opt. check by calc the eigenvecs of $\mathbf{A} \mathbf{A}^\top$.)
	\item normalize each col of $\mathbf{U}$ and $\mathbf{V}$.
\end{inparaenum}

\subsection{Low-Rank approximation}
Using only $K$ largest eigenvalues and corresponding eigenvectors. $\tilde{\mathbf{A}}_{i, j} = \sum_{k = 1}^K \mathbf{U}_{i, k} \mathbf{D}_{k,k} \mathbf{V}_{j, k} = \sum_{k = 1}^K \sigma_k u_{i,k} v_{j, k}$.\\
...
\begin{inparaenum}[\color{red} 1.]
	\item init: $\mathbf{x}^{(0)} \in \mathbb{R}^D$
	\item for $t = 0 \ \text{to} \ \mathit{maxIter}$: $\mathbf{x}^{(t+1)} = \mathbf{x}^{(t)} - \eta \nabla f(\mathbf{x}^{(t)})$, usually $\eta \approx \frac{1}{t}$
\end{inparaenum}
...
\begin{inparaenum}[\color{red} 1.]
	\item init: $\mathbf{x}^{(0)} \in \mathbb{R}^D$
	\item for $t = 0 \ \text{to} \ \mathit{maxIter}$:
	\item sample $n \overset{uni}{\sim} \{1, \ldots, N\}$
	\item $\mathbf{x}^{(t+1)} = \mathbf{x}^{(t)} - \eta \nabla f_n(\mathbf{x}^{(t)})$, usually stepsize $\eta \approx \frac{1}{t}$.
\end{inparaenum}
...
\textbf{Projected Gradient Update}: $\mathbf{x}^{(t+1)} = P_Q[\mathbf{x}^{(t)} - \eta \nabla f(\mathbf{x}^{(t)})]$,
...
\begin{compactdesc}
	...
	\item[Dual Problem:] $\max_{\boldsymbol{\lambda}, \boldsymbol{\nu}} D(\boldsymbol{\lambda}, \boldsymbol{\nu})$ s.t. $\boldsymbol{\lambda} \geq \mathbf{0}$. Note: $\max_{\boldsymbol{\lambda}, \boldsymbol{\nu}} D(\boldsymbol{\lambda}, \boldsymbol{\nu}) \le \min_\mathbf{x}{f(\mathbf{x})}$, equality if $dom\ f$ and $f$ convex. Use grad of $L$ to get opt min $x$, then insert into $D$ (using $-$).
\end{compactdesc}

\subsection{Convex Optimization}
$f : \mathbb{R}^D \rightarrow \mathbb{R}$ is convex, %if $dom\ f$ is a convex set, and 
if $\forall \mathbf{x}, \mathbf{y} \in dom\ f$, and for $0 \leq \alpha \leq 1$: $f(\alpha \mathbf{x} + (1 - \alpha)\mathbf{y}) \leq \alpha f(\mathbf{x}) + (1-\alpha)f(\mathbf{y})$. local=global min, \textbf{Convergence}: $f(\mathbf{x}^{(t)}) - f(\mathbf{x}^*) \le \frac{c}{t}$ (subgrad: $\frac{c}{\sqrt{t}}$).
\textbf{Subgrad:} $g \in \mathbb{R}^D$ of $f$ at $\mathbf{x}$: $f(\mathbf{y}) \geq f(\mathbf{x}) + g^\top(\mathbf{y}-\mathbf{x}) \ \forall \mathbf{y}$
...
\begin{compactdesc}
  ...
  \item[Latent Vector Model:] $w \mapsto (\mathbf{x}_w, b_w) \in \mathbb{R}^{D+1}$, $p_{\theta}(w|w') =\allowbreak \frac{\exp[\langle \mathbf{x}_w,\mathbf{x}_{w'}\rangle + b_w]}{\sum_{v\in V}{\exp[\langle \mathbf{x}_v,\mathbf{x}_{w'}\rangle + b_v ]}}$.
  Modifications:
  \begin{inparaitem}[\color{red}\textbullet]
  \item split vocab in main vocab $V$, context vocab $C$: $\log p_{\theta}(w|w') = \langle x_{w'},  y_{w} \rangle + b_w$,  word embed: $x_{w'}$, context embed: $y_w$
...
\begin{inparadesc}
	\item[\color{red}Target:] $\min_{\mathbf{U}, \mathbf{Z}} J(\mathbf{U}, \mathbf{Z}) = \|\mathbf{X} - \mathbf{U} \mathbf{Z}^\top\|_F^2 = \sum_{n=1}^N \sum_{k=1}^K z_{nk} \|\mathbf{x}_n - \mathbf{u}_k\|_2^2$, $\mathbf{Z} \in \mathbb{R}^{N \times K}$.
\end{inparadesc}
\begin{inparaenum}[\color{red} 1.]
	\item Initiate: choose $K$ centroids $\mathbf{U} = [\mathbf{u}_1, \ldots, \mathbf{u}_K]$ (usually u.a.r.)
	\item Assign data points to clusters. $k^\star(\mathbf{x}_n) = \argmin_k \{ \|\mathbf{x}_n - \mathbf{u}_k\|_2 \}$ returns cluster $k^\star$, whose centroid $\mathbf{u}_{k^\star}$ is closest to data point $\mathbf{x}_n$. Set $\mathbf{z}_{nk^\star} = 1$, and for $ l \neq k^\star~ \mathbf{z}_{nl}=0$.
	\item Update centroids: $\mathbf{u}_k = \frac{\sum_{n=1}^N z_{nk} \mathbf{x}_n}{\sum_{n=1}^N z_{nk}}$.
	\item Repeat from step 2, stops if $\|\mathbf{Z} - \mathbf{Z}^\text{new}\|_0 = \|\mathbf{Z} - \mathbf{Z}^\text{new}\|^2_F = 0$. $O(KND)$ ops. per round.
\end{inparaenum}
...
\subsubsection{Expectation-Maximization (EM) for GMM}
\begin{compactenum}[\color{red} 1.]
	...
	\item Repeat from (2.) with $t = t + 1$ if not $\| \log p(\mathbf{X} | \boldsymbol{\pi}^{(t)}, \boldsymbol{\mu}^{(t)}, \boldsymbol{\Sigma}^{(t)}) \allowbreak - \log p(\mathbf{X} | \boldsymbol{\pi}^{(t-1)}, \boldsymbol{\mu}^{(t-1)}, \boldsymbol{\Sigma}^{(t-1)}) \| < \epsilon$
\end{compactenum}

\subsubsection{Model Order Selection (AIC / BIC for GMM)}
...
\begin{inparaitem}[\color{red}\textbullet]
	...
	\item \#free params: fixed cov. matrix: $\kappa(\theta) = K \cdot D + (K - 1)$ ($K$: \#clusters, $D$: $\mathsf{dim}\text{(data)}=\mathsf{dim}(\mu_i)$, $K-1$: \#free clusters), full covariance matrix: $\kappa(\theta) = K(D + \frac{D(D+1)}{2}) + (K - 1)$.
	\item Compare AIC/BIC for different $K$ -- the smaller the better. %BIC penalizes complexity more.
\end{inparaitem}

\section{Convolutional Neural Networks}
\textbf{Neurons}: $F_\sigma(\mathbf{x};\mathbf{w}) = \sigma(w_0 + \sum_{i=1}^M{x_iw_i})$. \textbf{Output}: linear regression; $\mathbf{y} = \mathbf{W}^{(L)}\mathbf{x}^{(L-1)}$, binary classification; $y_1 = \text{P}[Y=1|\mathbf{x}] = \frac{1}{1 + \exp[-\langle \mathbf{w}_1^L,\mathbf{x}^{L-1}\rangle]}$, multiclass; $y_k = \text{P}[Y=k|\mathbf{x}]= \frac{\exp[\langle \mathbf{w}_k^L,\mathbf{x}^{L-1}\rangle]}{\sum_{m=1}^{K}{\exp[\langle \mathbf{w}_m^L, \mathbf{x}^{L-1}\rangle]}}$. \textbf{Loss function} $l(y, \hat{y})$: squared loss: $\frac{1}{2}(y - \hat{y})^2$, cross-entropy loss: $-y \log \hat{y} - (1-y)\log(1-\hat{y})$ (grad: if $y = 1$: $-\frac{1}{\hat{y}}$, if $y=0$: $\frac{1}{1-\hat{y}}$).
...
Choice of base depends on signal. Fourier for global (sin-like), wavelet for local support. PCA basis opt. for given $\Sigma$.

\subsection{Overcomplete Basis / Dictionaries}
...
\begin{compactdesc}
	\item[Mother func $\psi(t)$:] $1$ for $0 \leq t < \frac{1}{2}$, $-1$ for $\frac{1}{2} \leq t < 1$, $0$ else.
	\item[Haar sys:] $\psi_{n,k}(t) = 2^{n/2} \psi(2^n t - k)$, $2^{n/2}$ only for norm. vals!
\end{compactdesc}

$\mathbf{x} = \mathbf{Wa} = \mathbf{US}^{-1}\mathbf{a} \Rightarrow \mathbf{a} = \mathbf{SU}^\top \mathbf{x}$
...
\begin{compactitem}
	...
	\item $\min_{\mathbf{L},\mathbf{S}}\mathsf{rank}(\mathbf{L}) + \mu \lVert \mathbf{S}\rVert_0$, s. t. $\mathbf{L} + \mathbf{S} = \mathbf{X}$. As non-convex, use $\min_{\mathbf{L},\mathbf{S}} \|\mathbf{L}\|_\star + \lambda \lVert\mathbf{S}\rVert_1$ (PCP - \emph{not} the same in general)
...
\begin{inparaitem}[\color{red}\textbullet]
  ...
  \item ADMM for RPCA: $f_1(\mathbf{L}) = \|\mathbf{L}\|_\star$, $f_2(\mathbf{S}) = \lambda \| \mathbf{S} \|_1$, $\mathbf{A}_1 \mathbf{x}_1 + \mathbf{A}_2 \mathbf{x}_2 = \mathbf{b} \text{ becomes } \mathbf{L} + \mathbf{S} = \mathbf{X}$, therefore $L_p(\mathbf{L}, \mathbf{S}, \boldsymbol{\nu}) = \|\mathbf{L}\|_* + \nu \|\mathbf{S}\|_1 + \left< \nu, \mathrm{vec}(\mathbf{L}+\mathbf{S}-\mathbf{X}) \right> + \frac{P}{2} \| \mathbf{L}+ \mathbf{S} - \mathbf{X} \|_F^2$, $\mathbf{L}^{t+1} = \argmin_{\mathbf{L}} L_p(\mathbf{L}, \mathbf{S}^t, \mathbf{\lambda}^t)$, $\mathbf{S}^{t+1} = \argmin_{\mathbf{S}} L_p(\mathbf{L}^{t+1}, \mathbf{S}, \mathbf{\lambda}^t)$, $\mathbf{\lambda}^{t+1} = \mathbf{\lambda}^{t} + p \cdot \mathrm{vec}(\mathbf{L}^{t+1} + \mathbf{S}^{t+1} - \mathbf{X})$
